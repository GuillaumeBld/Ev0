...
async def fetch_league_players(league: str, season: str = "2025-2026") -> list[dict[str, Any]]:
    ...
    for player in players_data:
        players.append({
            ...
            "xg_per_90": round(float(player.get("xG", 0)) / max(int(player.get("time", 1)), 1) * 90, 3),
            "xa_per_90": round(float(player.get("xA", 0)) / max(int(player.get("time", 1)), 1) * 90, 3),
            "npxg_per_90": round(float(player.get("npxG", 0)) / max(int(player.get("time", 1)), 1) * 90, 3),
        })
...
async def fetch_team_players(team_slug: str, season: str = "2025-2026") -> list[dict[str, Any]]:
    ...
    for player in players_data:
        ...
        players.append({
            ...
            "shots": int(player.get("shots", 0)),
            "key_passes": int(player.get("key_passes", 0)),
            "xg_per_90": round(float(player.get("xG", 0)) / max(minutes, 1) * 90, 3) if minutes > 0 else 0,
            "xa_per_90": round(float(player.get("xA", 0)) / max(minutes, 1) * 90, 3) if minutes > 0 else 0,
        })
