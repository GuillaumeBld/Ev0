...
class TestOddsAPIClient:
    ...
    async def test_fetches_events(self, mock_get, client):
        mock_get.return_value = Mock(
            status_code=200, json=Mock(return_value=[{"id": "123", "home_team": "PSG"}])
        )
    ...
    async def test_fetches_player_props(self, mock_get, client, sample_response):
        mock_get.return_value = Mock(status_code=200, json=Mock(return_value=sample_response))

        odds = await client.get_player_props(
            "soccer_france_ligue_one", "abc123", "player_goal_scorer"
        )
