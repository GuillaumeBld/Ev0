...
export function Dashboard({ user }: DashboardProps) {
  ...
  return (
    <div className="p-4 md:p-8">
        ...
        <div className="mb-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            {breakdown.pnl_trend.length > 0 && (
              <div className="bg-ev-surface border border-ev-bd rounded-xl p-5">
                <h3 className="text-[10px] font-semibold text-ev-t4 uppercase tracking-[0.12em] mb-5">Courbe P&L</h3>
            ...
            <div className="space-y-4">
              {breakdown.by_market.length > 0 && (
                <div className="bg-ev-surface border border-ev-bd rounded-xl p-5">
                  <h3 className="text-[10px] font-semibold text-ev-t4 uppercase tracking-[0.12em] mb-4">Par marché</h3>
                  <BreakdownTable items={breakdown.by_market} />
                </div>
              )}
              {breakdown.by_league.length > 0 && (
                <div className="bg-ev-surface border border-ev-bd rounded-xl p-5">
                  <h3 className="text-[10px] font-semibold text-ev-t4 uppercase tracking-[0.12em] mb-4">Par ligue</h3>
      ...
      {/* Recommendations */}
      <div>
        <h2 className="text-[10px] font-semibold text-ev-t4 uppercase tracking-[0.15em] mb-4">
          Picks Value
        </h2>
        {isLoading ? (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {[1, 2].map((i) => (
              <div key={i} className="bg-ev-surface border border-ev-bd rounded-xl h-48 animate-pulse" />
            ))}
          </div>
          ...
          <div className="bg-ev-surface border border-ev-bd rounded-xl p-12 text-center">
            <AlertCircle className="w-8 h-8 text-ev-t5 mx-auto mb-3" />
            <p className="text-ev-t4 text-sm">
...
function DownloadCsvButton() {
  ...
  return (
    <button
      ...
      disabled={loading}
      className="flex items-center gap-2 px-4 py-2 bg-transparent border border-ev-bd hover:border-ev-bd disabled:opacity-50 text-ev-t3 hover:text-ev-t1 text-sm font-medium rounded-lg transition-all duration-200"
    >
...
function StatCard({ title, value, subtitle, icon: Icon, color, rawValue }: StatCardProps) {
  const valueColorClass = (() => {
    if (value === '—' || value === '…') return 'text-ev-t4'
  ...
  return (
    <div className="ev0-stat-card ev0-card-enter bg-ev-surface border border-ev-bd rounded-xl p-5">
      <div className="flex items-start justify-between">
        <div className="min-w-0">
          <p className="text-[10px] font-semibold text-ev-t4 uppercase tracking-[0.14em]">{title}</p>
          <p className={clsx('text-3xl font-mono font-semibold mt-2 tabular-nums leading-none', valueColorClass)}>
            {value}
          </p>
          <p className="text-[11px] text-ev-t5 mt-2">{subtitle}</p>
        </div>
...
function BreakdownTable({ items }: { items: BreakdownItem[] }) {
  return (
    <table className="w-full">
      <thead>
        <tr>
          <th className="text-left pb-2.5 text-[10px] font-semibold text-ev-t4 uppercase tracking-[0.1em]">Label</th>
          <th className="text-right pb-2.5 text-[10px] font-semibold text-ev-t4 uppercase tracking-[0.1em]">Paris</th>
          <th className="text-right pb-2.5 text-[10px] font-semibold text-ev-t4 uppercase tracking-[0.1em]">W/L</th>
          <th className="text-right pb-2.5 text-[10px] font-semibold text-ev-t4 uppercase tracking-[0.1em]">P&L</th>
          <th className="text-right pb-2.5 text-[10px] font-semibold text-ev-t4 uppercase tracking-[0.1em]">ROI</th>
        </tr>
      </thead>
      <tbody>
        {items.map((item) => (
          <tr key={item.label} className="border-t border-ev-bd2">
            <td className="py-2.5 text-sm text-ev-t2 font-medium capitalize">{item.label.replace('_', ' ')}</td>
            <td className="py-2.5 text-right text-sm text-ev-t4 font-mono">{item.bets}</td>
            <td className="py-2.5 text-right text-sm text-ev-t4 font-mono">{item.wins}/{item.losses}</td>
  ...
  return (
    <div>
      ...
      <div className="flex justify-between mt-3 text-[11px]">
        <span className="text-ev-t5 font-mono">{data[0].date}</span>
        ...
        <span className="text-ev-t5 font-mono">{lastPoint.date}</span>
      </div>
    </div>
  )
}
